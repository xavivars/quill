    ...
    if (refLine != null) {
      ...
      if (refLine.node.tagName === 'LI') {
        if (newLineNode.tagName === 'LI') {
          ...
          while (refLine && refLine.prev && refLine.node.tagName !== 'LI') {
            refLine = refLine.prev;
          }
        }
      }
      if (!dom(newLineNode.parentNode).isElement()) {
        parentNode.insertBefore(newLineNode, refNode);
      }
...
Editor = (function() {
  ...
  Editor.prototype.startPolling = function() {
    return this.timer = setInterval(_.bind(this.checkUpdate, this), this.options.pollInterval);
  };

  Editor.prototype.stopPolling = function() {
    ...
    _.each(formats, (function(_this) {
      return function(value, name) {
        ...
        if (format.isType(Format.types.LINE)) {
          if (format.config.exclude && _.find(format.config.exclude, function(candidateToExclude) {
            return _this.formats[candidateToExclude];
          })) {
            _.each(format.config.exclude, function(excludeFormatName) {
              var excludeFormat;
              excludeFormat = _this.doc.formats[excludeFormatName];
              if (excludeFormat != null) {
                _this.node = excludeFormat.remove(_this.node);
                return delete _this.formats[excludeFormatName];
              }
            });
          }
...
Normalizer = (function() {
  ...
  Normalizer.prototype.addFormat = function(config) {
    ...
    if (config.style != null) {
      this.whitelist.styles[config.style] = true;
    }
...
Selection = (function() {
  ...
  Selection.prototype._setNativeRange = function(startNode, startOffset, endNode, endOffset) {
    ...
    if (startNode != null) {
      if (dom.isIE(10) && !this.checkFocus()) {
        this.doc.root.focus();
      }
      nativeRange = this._getNativeRange();
  ...
  Keyboard.prototype._initEnter = function() {
    ...
    return this.addHotkey(keys, (function(_this) {
      return function(range, hotkey) {
        var delta, leaf, line, offset, ref, ref1, removeInheritedFormats, removeNonInheritedFormats;
        if (range == null) {
          return true;
        }
        ...
        removeInheritedFormats = _.reduce(line.formats, function(formats, value, name) {
          var format;
          format = _this.quill.editor.doc.formats[name];
          if (format && format.isType('line') && format.config.inherit) {
            formats[name] = false;
          }
          return formats;
        }, {});
        removeNonInheritedFormats = _.reduce(line.formats, function(formats, value, name) {
          var format;
          format = _this.quill.editor.doc.formats[name];
          if (format && format.isType('line') && !format.config.inherit) {
            formats[name] = false;
        ...
        if (range.isCollapsed() && line.length === 1 && Object.keys(removeInheritedFormats).length > 0) {
          delta.retain(1, removeInheritedFormats);
        } else {
          delta.insert('\n', line.formats)["delete"](range.end - range.start);
        }
        if (!leaf.next && offset === leaf.length) {
...
var Format, Quill, Toolbar, _, dom;
...
Toolbar = (function() {
  ...
  Toolbar.prototype._applyFormat = function(format, range, value) {
    ...
    return _.defer((function(_this) {
      return function() {
        var exclude, exclusive;
        exclude = _this.quill.editor.doc.formats[format].config.exclude;
        exclusive = exclude ? [].concat(exclude, format) : [format];
        _this.updateActive(range, exclusive);
...
Quill = (function(superClass) {
  ...
  Quill.prototype.getActiveFormats = function(start, end) {
    var leafFormats, lineFormats;
    if (start == null) {
      start = null;
    }
  ...
  Quill.prototype._getLeafFormats = function(start, end) {
    var contents, formats, line, offset, ref;
    if (start === end) {
      ref = this.editor.doc.findLineAt(start), line = ref[0], offset = ref[1];
      if (offset === 0) {
  ...
  Quill.prototype._getLineFormats = function(start, end) {
    var currLine, firstLine, formats, lastLine, offset, ref, ref1;
    formats = [];
    ref = this.editor.doc.findLineAt(start), firstLine = ref[0], offset = ref[1];
    ref1 = this.editor.doc.findLineAt(end), lastLine = ref1[0], offset = ref1[1];
    currLine = firstLine;
    while (true) {
  ...
  Quill.prototype._intersectFormats = function(formatGroups) {
    return _.reduce(formatGroups.slice(1), function(inAll, formats) {
      return _.omit(inAll, function(value, name) {
        return (formats == null) || (formats[name] == null) || formats[name] !== value;
      });
    }, formatGroups[0] || {});
  };
